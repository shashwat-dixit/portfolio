...
home_accept="$(curl -fsS -H 'Accept: text/markdown' "${SITE_URL}/")"
[[ "$home_accept" == \#* ]] || fail "Accept: text/markdown on / should return markdown"
echo "  Accept: text/markdown on / OK"

home_ua="$(curl -fsS -A 'ChatGPT-User/1.0' "${SITE_URL}/")"
[[ "$home_ua" == \#* ]] || fail "ChatGPT-User on / should return markdown, not HTML"
echo "  ChatGPT-User on / OK"
